ALTER TABLE results
  ADD COLUMN participation_points INTEGER NOT NULL DEFAULT 20;
...
BEGIN
  ...
  WITH ranked AS (
    SELECT
      ...
      COALESCE(SUM(r.points + r.participation_points), 0)::BIGINT AS total_points,
      COALESCE(SUM(r.points + r.participation_points) FILTER (WHERE r.tournament_id != v_latest_tournament_id), 0)::BIGINT AS previous_points,
      COUNT(r.id)::BIGINT AS attendances,
      COUNT(r.id) FILTER (WHERE r.position = 1)::BIGINT AS victories,
      COALESCE(SUM(r.points + r.participation_points) FILTER (WHERE r.tournament_id = v_latest_tournament_id), 0)::BIGINT AS points_today,
